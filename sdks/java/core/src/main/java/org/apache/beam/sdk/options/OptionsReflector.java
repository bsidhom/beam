...
/** Utilities to reflect over {@link Options}. */
class OptionsReflector {
   ...
   */
  static Set<OptionSpec> getOptionSpecs(Class<? extends Options> optionsInterface) {
    ...
    for (Map.Entry<String, Method> propAndGetter : propsToGetters.entries()) {
      ...
      Class<? extends Options> declaringClass =
          (Class<? extends Options>) getter.getDeclaringClass();

      if (!Options.class.isAssignableFrom(declaringClass)) {
        continue;
      }
      ...
      setBuilder.add(OptionSpec.of(declaringClass, prop, getter));
    }
